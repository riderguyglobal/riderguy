      ...
      <div className="flex items-center gap-1.5">
        <div className="relative flex-1">
          ...
          {hasSelection && !ac.retrieving && (
            <Check className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-brand-500" />
          )}
        ...
        <div className="absolute top-full left-0 right-12 mt-1.5 bg-white rounded-2xl border border-surface-200 shadow-xl z-30 max-h-72 overflow-y-auto animate-slide-up">
          {ac.results.map((suggestion) => {
            ...
            return (
              ...
              >
                <div className={`h-9 w-9 rounded-xl ${isCommunity ? 'bg-brand-100' : 'bg-surface-100'} flex items-center justify-center shrink-0 mt-0.5`}>
                  {typeIcon}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {isCommunity && (
                      <span className="shrink-0 text-[9px] font-semibold text-brand-600 bg-brand-100 px-1.5 py-0.5 rounded-full">
                        Community
                      </span>
                    )}
                  </div>
