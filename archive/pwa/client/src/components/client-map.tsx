...
export default function ClientMap({ pollEnabled = true }: ClientMapProps = {}) {
  ...
  const fetchNearbyRiders = useCallback(
    async (map: google.maps.Map, center: [number, number]) => {
      try {
        ...
        setRiderCount(riderMarkersRef.current.size);
      } catch {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Nearby rider count badge */}
      {mapReady && riderCount > 0 && (
        <div className="absolute bottom-3 left-3 z-10 flex items-center gap-1.5 rounded-lg bg-white/90 px-3 py-1.5 text-xs font-medium shadow-md backdrop-blur">
