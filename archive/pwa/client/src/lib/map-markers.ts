...
export function createRiderMarker(
  ...
  el.innerHTML = `
    <div style="position:relative;display:flex;align-items:center;justify-content:center;width:${size + 16}px;height:${size + 16}px;">
      ...
      ${showBike
        ? `<div data-rider-icon style="display:flex;align-items:center;justify-content:center;width:${size}px;height:${size}px;border-radius:14px;background:linear-gradient(135deg,${color},${adjustColor(color, -20)});box-shadow:0 4px 16px ${color}66;border:3px solid white;transition:transform 0.5s ease;">${SVG.bike}</div>`
...
export function updateRiderMarkerBearing(
  marker: google.maps.marker.AdvancedMarkerElement,
  bearing: number,
): void {
  const el = marker.content as HTMLElement;
  if (!el) return;
  const icon = el.querySelector<HTMLElement>('[data-rider-icon]');
  if (icon) {
