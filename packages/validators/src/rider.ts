  ...
  .superRefine((value, ctx) => {
    ...
    if (value.latitude === undefined) {
      ctx.addIssue({
        ...
        path: ['latitude'],
        message: 'Current latitude is required to go online',
      });
    }
    if (value.longitude === undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['longitude'],
        message: 'Current longitude is required to go online',
      });
    }
  });
...
export const createInHouseInvitationSchema = z
  .object({
    email: z.string().trim().email().optional(),
    phone: phoneSchema.optional(),
    expiresInDays: z.coerce.number().int().min(1).max(30).optional(),
    // The admin client keeps this UUID stable while retrying one issue
    // attempt. It is an operation identifier, never the invitation secret.
    idempotencyKey: z.string().uuid('A valid invitation request ID is required'),
  })
  .refine((value) => Boolean(value.email || value.phone), {
    message: 'An email address or phone number is required',
  })
  .refine((value) => !(value.email && value.phone), {
    message: 'Choose either email or SMS for one invitation request',
  });
...
export const listRiderOperationsCasesQuerySchema = z
  .object({
...
export const listRiderAuditHistoryQuerySchema = z
  .object({
    page: z.coerce.number().int().min(1).default(1),
...
export const listInHouseInvitationsQuerySchema = z
  .object({
    page: z.coerce.number().int().min(1).default(1),
    limit: z.coerce.number().int().min(1).max(100).default(20),
  })
...
export const revokeInHouseInvitationSchema = z
  .object({
    reason: z.string().trim().min(5, 'A meaningful revocation reason is required').max(500),
  })
  .strict();

export const reviewTrainingModuleSchema = z
  .object({
 ...
 */
export const createAssetFinancingInterestSchema = z
  .object({
...
export const updateAssetFinancingInterestStatusSchema = z
  .object({
...
export type ListInHouseInvitationsQuery = z.infer<typeof listInHouseInvitationsQuerySchema>;
...
export type UpdateAssetFinancingInterestStatusInput = z.infer<
  typeof updateAssetFinancingInterestStatusSchema
>;
