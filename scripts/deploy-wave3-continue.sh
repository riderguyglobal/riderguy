#!/bin/bash
# Wave 3 deploy continuation — migrations already applied; build + reload only.
set -euo pipefail
...
log "[2/8] Verify receiptEmailSentAt column"
sudo -u postgres psql riderguy_db -c '\d orders' | grep -i receiptemailsentat || { echo MISSING; exit 1; }

log "[3/8] Rebuilding shared packages"
npx turbo run build \
...
log "[8/8] Reloading rider + client + smoke check"
