...
async function api(token, pkg, method, p, body) {
  const res = await fetch(`https://androidpublisher.googleapis.com/androidpublisher/v3/applications/${pkg}${p}`, {
    method,
...
function rewrite(pkg, listing) {
  ...
  if (pkg === 'com.riderguy.client') {
    ...
    fullDescription = fullDescription
      .replace('• Pay securely with your in-app wallet, powered by Paystack.',
               '• Pay securely in-app with mobile money, card, or bank transfer — powered by Paystack.')
      .replace('• Secure in-app payments and wallet.',
               '• Secure in-app payments.')
      .replace('Track your delivery live on the map, and pay securely from your in-app wallet — no cash needed.',
               'Track your delivery live on the map, and pay securely in-app — no cash needed.')
      .replace('pay securely from your in-app wallet — no cash needed',
               'pay securely in-app — no cash needed');
  } else {
    fullDescription = fullDescription
      .replace('• Track your earnings, wallet, and withdrawals.',
...
(async () => {
  for (const app of APPS) {
    ...
    if (updated.fullDescription === listing.fullDescription && updated.shortDescription === listing.shortDescription) {
    ...
    try {
      ...
      console.log(`${app.pkg}: listing committed (NOT sent for review — press "Send for review" in the Console UI)`);
    }
