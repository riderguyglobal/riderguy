#!/usr/bin/env node
/**
 * Generate and upload Play Store graphics for the RiderGuy rider app.
 * - icon: 512x512 from apps/rider-native/assets/icon.png
...
async function buildIcon() {
  ...
  await sharp(path.join(ROOT, 'apps/rider-native/assets/icon.png'))
    .resize(512, 512)
    .png()
    .toFile(out);
  return out;
...
async function buildFeatureGraphic() {
  ...
  const logo = await sharp(path.join(ROOT, 'apps/rider-native/assets/icon.png'))
    .resize(300, 300)
    .png()
    .toBuffer();
...
(async () => {
  ...
  // Rider already has an approved icon + feature graphic — only refresh screenshots.
  const screenshotPaths = [];
  for (let i = 0; i < SCREENSHOTS.length; i++) {
    screenshotPaths.push(await buildScreenshot(SCREENSHOTS[i].file, SCREENSHOTS[i].caption, i));
  }
  console.log('Built:', screenshotPaths.map((p) => path.basename(p)).join(', '));
  ...
  console.log('Clearing old phone screenshots...');
  await jsonApi(token, 'DELETE', `/edits/${editId}/listings/en-US/phoneScreenshots`);
  ...
  console.log('Edit committed — rider store screenshots are live in Play Console.');
})().catch((err) => {
