...
export default function ContactInboxPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/contact', {
        params: { page, limit: 50, unread: filter === 'unread' ? 'true' : undefined },
      });
      const d = res.data.data;
      const nextPages = Math.max(1, d.pages ?? 1);
      setPages(nextPages);
      if (page > nextPages) {
        setPage(nextPages);
        return;
  ...
  const markRead = useCallback(
    async (id: string) => {
      try {
        ...
        setError(
          'The message was opened, but its read status could not be saved. Retry before leaving the inbox.',
        );
      }
    },
    [filter, load],
  ...
  const changeFilter = (nextFilter: 'all' | 'unread') => {
    setSelected(null);
    setPage(1);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">
            ...
            {unreadCount > 0 && (
              <span className="bg-brand-100 text-brand-700 ml-1.5 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold">
                {unreadCount} unread
              </span>
            )}
          </p>
                      ...
                      <li key={s.id}>
                        <button
                          ...
                          className={`w-full px-4 py-3.5 text-left transition-colors hover:bg-gray-50 ${
                            isSelected
                              ? 'border-brand-500 bg-brand-50/60 border-l-[3px]'
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-2">
                              ...
                              <p
                                className={`truncate text-sm ${!s.read ? 'font-semibold text-gray-900' : 'font-medium text-gray-600'}`}
                              >
                          ...
                          <p
                            className={`mt-0.5 truncate text-xs ${!s.read ? 'pl-4' : 'pl-0'} text-gray-400`}
                          >
                            {s.email}
                          </p>
                          <div className={`mt-1.5 ${!s.read ? 'pl-4' : 'pl-0'}`}>
                            <span
                              className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-medium ${meta.color}`}
                            >
                              {meta.label}
                            </span>
                          </div>
                          <p
                            className={`mt-1 line-clamp-1 text-xs text-gray-400 ${!s.read ? 'pl-4' : 'pl-0'}`}
                          >
                            {s.message}
                          </p>
                        </button>
              ...
              {pages > 1 && (
                <div className="flex items-center justify-between border-t border-gray-100 px-4 py-3">
                  <button
                    ...
                    className="hover:border-brand-300 hover:text-brand-700 min-h-11 rounded-lg border border-gray-200 px-3 text-sm font-semibold text-gray-700 transition-colors disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Previous
                  </button>
              ...
              <div className="rounded-xl border border-gray-200 bg-white">
                ...
                <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-6 py-5">
                  <div className="flex items-center gap-3">
                    <div className="bg-brand-100 text-brand-700 flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-base font-bold">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
                  ...
                  <a
                    ...
                    className="bg-brand-500 shadow-brand-500/30 hover:bg-brand-600 inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all active:scale-[0.98]"
                  >
