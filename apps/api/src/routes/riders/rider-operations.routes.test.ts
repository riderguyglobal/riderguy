...
import {
  describeRiderReadiness,
  RiderOperationsService,
} from '../../services/rider-operations.service';
import { OnboardingService } from '../../services/onboarding.service';
import {
  listInHouseInvitationsHandler,
...
describe('Rider Operations routes', () => {
  ...
  it('registers the summary, case queue, case detail, history, and invitation controls', () => {
    ...
    expect(stack.some((layer) => layer.route?.path === '/cases' && layer.route.methods.get)).toBe(
      true,
    );
    expect(
      stack.some((layer) => layer.route?.path === '/cases/:riderId' && layer.route.methods.get),
    ).toBe(true);
    expect(
      stack.some(
        (layer) => layer.route?.path === '/cases/:riderId/history' && layer.route.methods.get,
      ),
    ).toBe(true);
    expect(
      stack.some((layer) => layer.route?.path === '/invitations' && layer.route.methods.get),
    ).toBe(true);
    expect(
      stack.some(
        (layer) =>
          layer.route?.path === '/invitations/:invitationId/revoke' && layer.route.methods.patch,
      ),
    ).toBe(true);
  });
  ...
  it('returns invitation pagination metadata and passes validated paging input', async () => {
    const page = {
      items: [{ id: 'invite-21' }],
      pagination: { page: 2, limit: 20, total: 21, totalPages: 2 },
    };
    const list = vi
      .spyOn(OnboardingService, 'listInHouseInvitations')
      .mockResolvedValue(page as never);
    const query = { page: 2, limit: 20 };
    const response = responseDouble();

    await listInHouseInvitationsHandler({ query } as unknown as Request, response.response);
...
describe('Rider Operations validation', () => {
  it('normalizes safe defaults and rejects unbounded or unknown queue input', () => {
    expect(listRiderOperationsCasesQuerySchema.parse({})).toEqual({
      queue: 'PENDING',
      page: 1,
      limit: 20,
    });
    expect(() =>
      listRiderOperationsCasesQuerySchema.parse({ queue: 'DELETE_EVERYTHING' }),
    ).toThrow();
    ...
    expect(listInHouseInvitationsQuerySchema.parse({ page: '2', limit: '50' })).toEqual({
      page: 2,
      limit: 50,
    });
    expect(() => listInHouseInvitationsQuerySchema.parse({ limit: 101 })).toThrow();
  });
  ...
  it('requires reasons for destructive invitation and training actions', () => {
    ...
    expect(revokeInHouseInvitationSchema.parse({ reason: 'Wrong recipient' })).toEqual({
      reason: 'Wrong recipient',
    });
    expect(() => reviewTrainingModuleSchema.parse({ decision: 'REVOKED' })).toThrow();
    expect(reviewTrainingModuleSchema.parse({ decision: 'VERIFIED' })).toEqual({
      decision: 'VERIFIED',
    });
    expect(() => reviewDocumentSchema.parse({ status: 'REJECTED' })).toThrow();
    expect(() =>
      reviewDocumentSchema.parse({ status: 'APPROVED', rejectionReason: 'Not relevant' }),
    ).toThrow();
  });
});
