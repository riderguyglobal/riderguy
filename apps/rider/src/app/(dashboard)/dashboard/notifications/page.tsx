...
import { Bell, CheckCheck, ChevronLeft, Package, DollarSign, AlertTriangle, Star, Megaphone, Award } from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-page animate-page-enter">
      {/* Header */}
      <div className="safe-area-top bg-card sticky top-0 z-20 border-b border-themed">
        ...
        {/* Filter tabs */}
        <div className="flex mx-5 mb-3 bg-surface-100 dark:bg-surface-800 rounded-xl p-1">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 rounded-full bg-surface-100 dark:bg-surface-800 flex items-center justify-center mb-4">
              <Bell className="h-7 w-7 text-surface-300 dark:text-surface-600" />
            </div>
            <p className="text-secondary font-medium">
              {filter === 'unread' ? 'All caught up!' : 'No notifications yet'}
            </p>
            <p className="text-subtle text-sm mt-1">
              {filter === 'unread' ? 'You have no unread notifications' : "We'll notify you about job offers and earnings"}
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className={`text-sm truncate ${
                        n.isRead ? 'font-medium text-secondary' : 'font-semibold text-primary'
                      }`}>{n.title}</p>
