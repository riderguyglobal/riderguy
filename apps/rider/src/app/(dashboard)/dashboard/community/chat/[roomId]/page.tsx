...
export default function ChatRoomPage() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0a0e17]">
      {/* Header */}
      <div className="sticky top-0 z-40 bg-[#0a0e17]/80 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="px-4 pt-[env(safe-area-inset-top)]">
          <div className="flex items-center justify-between h-14">
            ...
            <button className="p-2 -mr-2 text-surface-400 hover:text-surface-200">
            ...
            <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} ${showAvatar ? 'mt-3' : 'mt-0.5'}`}>
              ...
              {!isOwn && (
                <div className="w-8 flex-shrink-0">
                  {showAvatar && (
                    <div className="h-7 w-7 rounded-full bg-surface-700 flex items-center justify-center text-[10px] font-bold text-surface-300 overflow-hidden">
                      {msg.senderAvatar ? (
              ...
              <div className={`max-w-[75%] ${isOwn ? 'items-end' : 'items-start'}`}>
                {showName && (
                  <p className="text-[10px] text-surface-500 mb-0.5 ml-1">{msg.senderName}</p>
                )}
                <div
                  className={`px-3 py-2 rounded-2xl text-sm leading-relaxed ${
                    isOwn
                      ? 'bg-brand-500 text-white rounded-br-md'
                      : 'bg-white/[0.06] text-surface-200 rounded-bl-md'
                  }`}
                >
                  {msg.content}
                </div>
                <p className={`text-[9px] mt-0.5 ${isOwn ? 'text-right' : 'text-left'} text-surface-600`}>
      ...
      {typingNames.length > 0 && (
        <div className="px-4 pb-1">
          <p className="text-[11px] text-surface-400 animate-pulse">
        ...
        <div className="flex items-center gap-2 px-4 py-3">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full bg-white/[0.06] border border-white/[0.08] rounded-full px-4 py-2.5 text-sm text-white placeholder:text-surface-500 focus:outline-none focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/30"
            />
          </div>
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="h-10 w-10 rounded-full bg-brand-500 flex items-center justify-center text-white shadow-lg shadow-brand-500/30 disabled:opacity-40 disabled:shadow-none transition-all btn-press"
          >
