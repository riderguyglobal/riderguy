...
export default function CommunityChatScreen() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        Toast.show({ type: 'error', text1: error?.response?.data?.error?.message ?? 'Could not open room.' });
      }
    };
    start();
    return () => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: riderColors.surface }}>
      <RiderHeader title="Crew room" subtitle="Live rider chat" canGoBack right={<StatusPill status="ONLINE" label={`${messages.length}`} />} />
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={{ maxWidth: '82%', alignSelf: isMine ? 'flex-end' : 'flex-start' }}>
                {!isMine ? <Text style={{ color: riderColors.muted, fontSize: 11, fontWeight: '800', marginBottom: 4, paddingHorizontal: 4 }}>{item.senderName}</Text> : null}
                <View style={{ backgroundColor: isMine ? riderColors.green : riderColors.white, borderRadius: 18, borderBottomRightRadius: isMine ? 5 : 18, borderBottomLeftRadius: isMine ? 18 : 5, paddingHorizontal: 14, paddingVertical: 10, borderWidth: isMine ? 0 : 1, borderColor: riderColors.line }}>
