...
export default function ForumScreen() {
  ...
  const createPost = useMutation({
    mutationFn: async () =>
      api.post('/community/forum/posts', {
        title: title.trim(),
        body: body.trim(),
        category: 'GENERAL',
      }),
    ...
    onError: (error: any) =>
      Toast.show({
        type: 'error',
        text1: error?.response?.data?.error?.message ?? 'Could not publish.',
      }),
  });

  const vote = useMutation({
    mutationFn: async ({ postId, value }: { postId: string; value: 1 | -1 }) =>
      api.post(`/community/forum/posts/${postId}/vote`, { value }),
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: riderColors.surface }}>
      <RiderHeader
        ...
        right={
          ...
          >
            <Ionicons name="add" size={24} color={riderColors.white} />
          </TouchableOpacity>
        }
      />
        ...
        ListEmptyComponent={
          !isLoading ? (
            <EmptyState
              ...
              title="No posts yet"
              body="Start the first useful rider discussion in this space."
            />
          ) : null
        }
        renderItem={({ item }) => (
          <RiderCard>
            ...
            <Text
              style={{ color: riderColors.muted, fontSize: 13, lineHeight: 20, marginTop: 7 }}
              numberOfLines={3}
            >
              {item.body}
            </Text>
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 14 }}>
              <TouchableOpacity
                onPress={() => vote.mutate({ postId: item.id, value: 1 })}
                style={{
                  flexDirection: 'row',
                  alignItems: 'center',
                  gap: 5,
              ...
              >
                ...
                <Text style={{ color: riderColors.ink, fontWeight: '900', fontSize: 12 }}>
                  {item.upvotes ?? item.score ?? 0}
                </Text>
              </TouchableOpacity>
              <View
                style={{
              ...
              >
                ...
                <Text style={{ color: riderColors.ink, fontWeight: '900', fontSize: 12 }}>
                  {item.commentCount ?? item._count?.comments ?? 0}
                </Text>
              </View>
              {item.author?.id !== user?.id ? (
                ...
                >
                  <Ionicons name="flag-outline" size={15} color={riderColors.red} />
                  <Text style={{ color: riderColors.red, fontWeight: '800', fontSize: 12 }}>
                    Report
                  </Text>
                </TouchableOpacity>
              ) : null}
          ...
          >
            ...
            >
              ...
              <TouchableOpacity onPress={() => setComposerOpen(false)}>
                <Ionicons name="close" size={24} color={riderColors.ink} />
              </TouchableOpacity>
            </View>
            <RiderTextField
              label="Title"
