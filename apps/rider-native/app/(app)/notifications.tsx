...
export default function NotificationsScreen() {
  ...
  const openNotification = async (item: any) => {
    ...
    const target = resolveRiderNotificationRoute(item.data, item.type);
        ...
        ListEmptyComponent={
          !isLoading ? (
            <EmptyState
              ...
              body="Important rider updates will collect here."
            />
          ) : null
        }
        renderItem={({ item }) => {
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
                  <View
                    style={{
                      ...
                      height: 42,
                      borderRadius: 15,
                      backgroundColor: isUnread ? riderColors.greenSoft : riderColors.panelAlt,
                      alignItems: 'center',
                      justifyContent: 'center',
                    }}
                  >
                    <Ionicons
                      name={isUnread ? 'radio' : 'notifications-outline'}
                      size={19}
                      color={isUnread ? riderColors.greenDark : riderColors.muted}
                    />
                  </View>
                  <View style={{ flex: 1 }}>
