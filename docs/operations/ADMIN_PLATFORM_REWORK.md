...
| Medium | Production storage | The running API reports the storage integration as incomplete and is using the local-disk fallback. This is functional on the current single server but not horizontally durable. | Complete the S3/R2 credential set before horizontal scaling; keep the safe fallback signal visible to Super Admins. |
